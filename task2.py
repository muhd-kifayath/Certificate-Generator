        ...
        if file:
            try:
                ...
                for row in reader:
                    ...
                    if not event:
                        flash(f"Certificate Generator says: Event with ID {event_id} does not exist. "
                              f"Skipping participant {participant_name}.", 'warning')
                        continue

                    participant = Participant(eventid=event_id, participant_name=participant_name,
